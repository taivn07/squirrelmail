...
function sqimap_read_data_list ($imap_stream, $tag_uid, $handle_errors,
    ...
    if ($read === false) {
        ...
        if ($query != '' &&  $cmd != 'login') {
            $string .= ("Query:") . ' '. sq_htmlspecialchars($query)
            . '<br />' . "</font><br />\n";
        }
        error_box($string,$color);
        exit;
    ...
    case 'NO':
        ...
        if (strstr($message, 'command resulted in') === false) {
            ...
            $string = "<b><font color=\"$color[2]\">\n" .
                ...
                "</b><br />\n" .
                _("Query:") . ' ' .
                sq_htmlspecialchars($query) . '<br />' .
                _("Reason Given:") . ' ' .
                sq_htmlspecialchars($message) . "</font><br />\n";
            error_box($string,$color);
            echo '</body></html>';
            exit;
        }
    ...
    if ($response != 'OK') {
        if (!$hide) {
            if ($response != 'NO') {
                ...
                $message = sq_htmlspecialchars($message);
                set_up_language($squirrelmail_language, true);
                require_once(SM_PATH . 'functions/display_messages.php');
                if ($response == 'BAD') {
                ...
                if (isset($read) && is_array($read)) {
                    ...
                    foreach ($read as $line) {
                        $string .= sq_htmlspecialchars($line) . "<br />\n";
                    }
                }
                error_box($string,$color);
                exit;
