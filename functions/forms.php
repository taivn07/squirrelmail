...
function addInputField($type, $name = null, $value = null, $attributes = '') {
    return '<input type="'.$type.'"'.
        ($name  !== null ? ' name="'.sq_htmlspecialchars($name).'"'   : '').
        ($value !== null ? ' value="'.sq_htmlspecialchars($value).'"' : '').
        $attributes . " />\n";
}
...
function addSelect($name, $values, $default = null, $usekeys = false)
{
    ...
    $ret = '<select name="'.sq_htmlspecialchars($name) . "\">\n";
    foreach ($values as $k => $v) {
        if(!$usekeys) $k = $v;
        $ret .= '<option value="' .
            sq_htmlspecialchars( $k ) . '"' .
            (($default == $k) ? ' selected="selected"' : '') .
            '>' . sq_htmlspecialchars($v) ."</option>\n";
    }
    $ret .= "</select>\n";
...
function addTextArea($name, $text = '', $cols = 40, $rows = 10, $attr = '') {
    return '<textarea name="'.sq_htmlspecialchars($name).'" '.
        'rows="'.(int)$rows .'" cols="'.(int)$cols.'" '.
        $attr . '>'.sq_htmlspecialchars($text) ."</textarea>\n";
}
...
function addForm($action, $method = 'post', $name = '', $enctype = '', $charset = '', $extra = '', $add_token = FALSE)
{
    ...
    if($charset) {
        $charset = ' accept-charset="'.sq_htmlspecialchars($charset).'"';
    }
