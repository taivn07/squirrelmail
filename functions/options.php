        ...
        if ($radio_buttons) {
            ...
            foreach ($this->possible_values as $real_value => $disp_value) {
                $result .= "\n" . '<input type="radio" name="new_' . $this->name 
                         . '" id="new_' . $this->name . '_' 
                         . ($this->htmlencoded ? $real_value : sq_htmlspecialchars($real_value))
                         . '" value="'
                         . ($this->htmlencoded ? $real_value : sq_htmlspecialchars($real_value))
                         ...
                         . ($this->htmlencoded ? $real_value : sq_htmlspecialchars($real_value))
                         . '">'
                         . ($this->htmlencoded ? $disp_value : sq_htmlspecialchars($disp_value))
                         . '</label>';
...
//FIXME: not sure about these sizes... seems like we could add another on the "large" side...
        ...
        foreach ($this->possible_values as $real_value => $disp_value) {
            ...
            $new_option .= '>' . ($this->htmlencoded ? $disp_value : sq_htmlspecialchars($disp_value)) . "</option>\n";
            /* And add the new option string to our select tag. */
            $result .= $new_option;
        }

        /* Close the select tag and return our happy result. */
        $result .= '</select>' . sq_htmlspecialchars($this->trailing_text) . "\n";
        return $result;
    }
...
//FIXME: not sure about these sizes... seems like we could add another on the "large" side...
        ...
        foreach ($this->possible_values as $real_value => $disp_value) {
            ...
            if ( is_array($disp_value) ) {
                ...
                /* Start the next new option string. */
                $new_option = '<option value="' . sq_htmlspecialchars($real_value) . '"';
                ...
                /* Add the display value to our option string. */
                $new_option .= '>' . sq_htmlspecialchars($disp_value) . "</option>\n";
            }
            /* And add the new option string to our select tag. */
            $result .= $new_option;
        }
    ...
    function createWidget_TextArea() {
        ...
        $result = "<textarea name=\"new_$this->name\" rows=\"$rows\" "
                ...
                . sq_htmlspecialchars($this->value) . "</textarea>\n";
        return ($result);
    }
    ...
    function createWidget_Boolean($checkbox=TRUE) {
        ...
        if ($checkbox) {
            $result = '<input type="checkbox" name="new_' . $this->name
                    ...
                    . sq_htmlspecialchars($this->trailing_text) . '</label>';
        }
        ...
        //
            ...
            $yes_option = '<input type="radio" name="new_' . $this->name 
                        ...
                        . '<label for="new_' . $this->name . '_yes">' . (!empty($this->yes_text) ? sq_htmlspecialchars($this->yes_text) : _("Yes")) . '</label>';

            /* Build the no choice. */
            $no_option = '<input type="radio" name="new_' . $this->name
                       . '" id="new_' . $this->name . '_no"'
                       . ' value="' . SMPREF_NO . "\"$no_chk $this->script />&nbsp;"
                       . '<label for="new_' . $this->name . '_no">' . (!empty($this->no_text) ? sq_htmlspecialchars($this->no_text) : _("No")) . '</label>';
    ...
    function createWidget_Hidden() {
        $result = '<input type="hidden" name="new_' . $this->name
                . '" value="' . sq_htmlspecialchars($this->value)
                . '" ' . $this->script . ' />';
        return ($result);
    }
        ...
        switch ($this->layout_type) {
            case SMOPT_EDIT_LIST_LAYOUT_SELECT:
                ...
                foreach ($this->possible_values as $value) {
                    ...
                    // Start the next new option string.
                    //
                    $result .= '<option value="' . sq_htmlspecialchars($value) . '"';
                    ...
                    // Add the display value to our option string.
                    //
                    $result .= '>' . sq_htmlspecialchars($value) . "</option>\n";
            ...
            case SMOPT_EDIT_LIST_LAYOUT_LIST:
                ...
                foreach ($this->possible_values as $key => $value) {
                    ...
                    $result .= '<tr bgcolor="' . $color[$bgcolor] . '">'
                             . '<td width="1%"><input type="checkbox" name="new_' . $this->name . '[' . ($index++) . ']" id="' . $this->name . '_list_item_' . $key . '" value="' . sq_htmlspecialchars($value) . '"></td>'
                             . '<td><label for="' . $this->name . '_list_item_' . $key . '">' . sq_htmlspecialchars($value) . '</label></td>'
                             . "</tr>\n";
    ...
    function createWidget_Submit() {
        ...
        $result = "<input type=\"submit\" name=\"$this->name\" value=\""
                . sq_htmlspecialchars($this->comment)
                . "\" $this->script />" 
                . sq_htmlspecialchars($this->trailing_text) . "\n";
...
function create_hidden_element($name, $value) {
    $result = '<input type="hidden" '
            . 'name="' . $name . '" '
            . 'value="' . sq_htmlspecialchars($value) . '" />';
    return ($result);
}
