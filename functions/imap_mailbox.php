...
function sqimap_mailbox_select ($imap_stream, $mailbox) {
    ...
    if (strstr($mailbox, '../') || substr($mailbox, 0, 1) == '/') {
        ...
        error_box(sprintf(_("Invalid mailbox name: %s"),sq_htmlspecialchars($mailbox)),$color);
    ...
    foreach ($boxes as $boxes_part) {
        if ($flag == NULL || !in_array($flag, $boxes_part['flags'])) {
            ...
            if (strtolower($box) == 'inbox') { // inbox is special and not casesensitive
                ...
                switch ($shorten_box_names)
                {
                  ...
                  default:  /* default, long names, style = 0 */
                    $box2 = str_replace(' ', '&nbsp;', sq_htmlspecialchars(imap_utf7_decode_local($boxes_part['unformatted-disp'])));
                    break;
                }
            }
            ...
            if ($show_selected != 0 && in_array($lowerbox, $show_selected) ) {
                $mbox_options .= '<option value="' . sq_htmlspecialchars($box) .'" selected="selected">'.$box2.'</option>' . "\n";
            } else {
                $mbox_options .= '<option value="' . sq_htmlspecialchars($box) .'">'.$box2.'</option>' . "\n";
            }
        }
    }
    return $mbox_options;
