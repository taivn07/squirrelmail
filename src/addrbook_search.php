...
function display_result($res, $includesource = true) {
    ...
    while (list($undef, $row) = each($res)) {
        $email = sq_htmlspecialchars(addcslashes(AddressBook::full_address($row), "'"), ENT_QUOTES);
        if ($line % 2) { 
            $tr_bgcolor = $color[12];
        } else {
            $tr_bgcolor = $color[4];
        ...
        html_tag( 'td', '&nbsp;' . sq_htmlspecialchars($row['name']), 'left', '', 'valign="top" nowrap' ) .
        html_tag( 'td', '&nbsp;' .
             '<a href="javascript:to_and_close(' .
                 "'" . $email . "');\">" . sq_htmlspecialchars($row['email']) . '</a>'
        , 'left', '', 'valign="top"' ) .
        html_tag( 'td', sq_htmlspecialchars($row['label']), 'left', '', 'valign="top" nowrap' );
        if ($includesource) {
