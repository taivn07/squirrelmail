...
if ($vcard_nice['version'] == '2.1') {
    ...
    echo '<tr><td align="center">' .
         sprintf(_("vCard Version %s is not supported. Some information might not be converted correctly."),
                 sq_htmlspecialchars($vcard_nice['version'])) .
         "</td></tr>\n";
...
if (isset($vcard_nice['url'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['url']) .
        '">' . _("Web Page") . "</option>\n";
}
if (isset($vcard_nice['adr'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['adr']) .
        '">' . _("Address") . "</option>\n";
}
if (isset($vcard_nice['title'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['title']) .
        '">' . _("Title") . "</option>\n";
}
if (isset($vcard_nice['org'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['org']) .
        '">' . _("Organization / Department") . "</option>\n";
}
if (isset($vcard_nice['title'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['title']) .
        '; ' . sq_htmlspecialchars($vcard_nice['org']) .
        '">' . _("Title &amp; Org. / Dept.") . "</option>\n";
}
if (isset($vcard_nice['tel;work'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['tel;work']) .
        '">' . _("Work Phone") . "</option>\n";
}
if (isset($vcard_nice['tel;home'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['tel;home']) .
        '">' . _("Home Phone") . "</option>\n";
}
if (isset($vcard_nice['tel;cell'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['tel;cell']) .
        '">' . _("Cellular Phone") . "</option>\n";
}
if (isset($vcard_nice['tel;fax'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['tel;fax']) .
        '">' . _("Fax") . "</option>\n";
}
if (isset($vcard_nice['note'])) {
    echo '<option value="' . sq_htmlspecialchars($vcard_nice['note']) .
        '">' . _("Note") . "</option>\n";
}
echo '</select>';
?>
...
echo '<input name="addaddr[email]" type="hidden" value="' .
        sq_htmlspecialchars(!empty($vcard_nice['email;internet'])?$vcard_nice['email;internet']:'') . '" />' .
