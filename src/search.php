...
function printSearchMessages($msgs,$mailbox, $cnt, $imapConnection, $where, $what, $usecache = false, $newsort = false) {
    ...
    if ($cnt > 0) {
        ...
        echo html_tag( 'div', '<b><big>' . _("Folder:") . ' '.
            sq_htmlspecialchars($showbox) .'</big></b>','center') . "\n";
        ...
        echo '<form name="' . $form_name . '" method="post" action="move_messages.php">' ."\n" .
             '<input type="hidden" name="mailbox" value="'.sq_htmlspecialchars($mailbox).'">' . "\n" .
...
if ($saved_count > 0) {
    ...
    for ($i=0; $i < $saved_count; ++$i) {
        ...
        echo html_tag( 'td', sq_htmlspecialchars(imap_utf7_decode_local($saved_attributes['saved_folder'][$i + 1])), 'left', '', 'width="35%"' )
        . html_tag( 'td', sq_htmlspecialchars($saved_attributes['saved_what'][$i + 1]), 'left' )
        . html_tag( 'td', sq_htmlspecialchars($saved_attributes['saved_where'][$i + 1]), 'center' )
        . html_tag( 'td', '', 'right' )
        .   '<a href="search.php'
...
if ($recent_count > 0) {
    ...
    for ($i=1; $i <= $recent_count; ++$i) {
            ...
            echo html_tag( 'td', sq_htmlspecialchars(imap_utf7_decode_local($attributes['search_folder'][$i])), 'left', '', 'width="35%"' )
               . html_tag( 'td', sq_htmlspecialchars($attributes['search_what'][$i]), 'left' )
               . html_tag( 'td', sq_htmlspecialchars($attributes['search_where'][$i]), 'center' )
               . html_tag( 'td', '', 'right' )
