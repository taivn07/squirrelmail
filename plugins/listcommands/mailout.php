...
case 'unsubscribe':
    ...
    break;
default:
    error_box(sprintf(_("Unknown action: %s"),sq_htmlspecialchars($action)), $color);
    exit;
}
...
printf( $out_string, sq_htmlspecialchars($send_to) );
...
if (count($idents) > 1) {
    ...
    echo sq_htmlspecialchars('"'.$idents[0]['full_name'].'" <'.$idents[0]['email_address'].'>');
}

echo '<br /><br />'
. '<input type="hidden" name="send_to" value="' . sq_htmlspecialchars($send_to) . '">'
. '<input type="hidden" name="subject" value="' . sq_htmlspecialchars($subject) . '">'
. '<input type="hidden" name="body" value="' . sq_htmlspecialchars($body) . '">'
