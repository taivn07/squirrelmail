...
function Mail_Fetch_Status($msg) {
    echo html_tag( 'table',
             html_tag( 'tr',
                 html_tag( 'td', sq_htmlspecialchars( $msg ) , 'left' )
                     ),
                   '', '', 'width="90%"' );
    flush();
}
...
if (! sqgetGlobalVar('server_to_fetch', $server_to_fetch, SQ_POST) ) {
    ...
    for ($i=0;$i<$mailfetch_server_number;$i++) {
        echo "<option value=\"$i\">" .
            sq_htmlspecialchars((($mailfetch_alias_[$i]=='')?$mailfetch_server_[$i]:$mailfetch_alias_[$i])) .
            '</option>' . "\n";
    }
    echo            '</select>' .
        '</td>' .
...
for ($i_loop=$i_start;$i_loop<$i_stop;$i_loop++) {
        ...
        html_tag( 'table',
            html_tag( 'tr',
                html_tag( 'td', '<b>' . _("Fetching from ") .
                    sq_htmlspecialchars((($mailfetch_alias_[$i_loop] == '')?$mailfetch_server:$mailfetch_alias_[$i_loop])) .
                    '</b>',
                'center' ) ,
            '', $color[9] ) ,
        '', '', 'width="90%"' );
    ...
    for (; $i <= $Count; $i++) {
        ...
        while ( (!$MessArray) or (gettype($MessArray) != "array")) {
            ...
            $Count = $pop3->login($mailfetch_user, $mailfetch_pass);
            if (($Count == false || $Count == -1) && $pop3->ERROR != '') {
                Mail_Fetch_Status(_("Login Failed:") . ' ' . sq_htmlspecialchars($pop3->ERROR) );
                Mail_Fetch_Status(_("Saving UIDL"));
                setPref($data_dir,$username,"mailfetch_uidl_$i_loop", $mailfetch_uidl[$i-1]);
        ...
        if (substr($Line, 0, 1) == '+') {
            ...
            if ($mailfetch_lmos != 'on') {
                if( $pop3->delete($i) ) {
                    ...
                    Mail_Fetch_Status(_("Delete failed:") . sq_htmlspecialchars($pop3->ERROR) );
                }
            }
        } else {
            echo "$Line";
